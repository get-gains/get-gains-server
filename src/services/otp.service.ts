import crypto from 'crypto';
import prisma from '../config/database';
import { logger } from '../utils/logger';
import { BadRequestException } from '../lib/errors';
import { sendPasswordResetCode } from './email.service';

const OTP_CHARSET = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
const OTP_LENGTH = 6;
const OTP_EXPIRY_MINUTES = 10;
const OTP_COOLDOWN_SECONDS = 60;
const MAX_OTP_PER_HOUR = 5;

const generateCode = (): string => {
  let code = '';
  const bytes = crypto.randomBytes(OTP_LENGTH);
  for (let i = 0; i < OTP_LENGTH; i++) {
    code += OTP_CHARSET[bytes[i] % OTP_CHARSET.length];
  }
  return code;
};

export const generateOtp = async (
  email: string
): Promise<{ success: boolean; cooldownRemaining?: number }> => {
  const normalizedEmail = email.toLowerCase();

  const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000);
  const recentCount = await prisma.password_reset_otp.count({
    where: {
      email: normalizedEmail,
      created_at: { gte: oneHourAgo },
    },
  });

  if (recentCount >= MAX_OTP_PER_HOUR) {
    throw new BadRequestException(
      'AUTH_OTP_RATE_LIMITED',
      'Too many code requests. Please try again later.'
    );
  }

  const existingOtp = await prisma.password_reset_otp.findFirst({
    where: {
      email: normalizedEmail,
      verified: false,
      cooldown_until: { gt: new Date() },
    },
    orderBy: { created_at: 'desc' },
  });

  if (existingOtp?.cooldown_until) {
    const remainingSeconds = Math.ceil(
      (existingOtp.cooldown_until.getTime() - Date.now()) / 1000
    );
    return { success: false, cooldownRemaining: Math.max(1, remainingSeconds) };
  }

  const code = generateCode();
  const expiresAt = new Date(Date.now() + OTP_EXPIRY_MINUTES * 60 * 1000);
  const cooldownUntil = new Date(Date.now() + OTP_COOLDOWN_SECONDS * 1000);

  await prisma.password_reset_otp.create({
    data: {
      email: normalizedEmail,
      code,
      expires_at: expiresAt,
      cooldown_until: cooldownUntil,
    },
  });

  logger.info('OTP generated', { email: normalizedEmail });

  await sendPasswordResetCode(email, code);

  return { success: true };
};

export const verifyOtp = async (
  email: string,
  code: string
): Promise<string> => {
  const normalizedEmail = email.toLowerCase();
  const normalizedCode = code.toUpperCase();

  const otp = await prisma.password_reset_otp.findFirst({
    where: {
      email: normalizedEmail,
      verified: false,
      expires_at: { gt: new Date() },
    },
    orderBy: { created_at: 'desc' },
  });

  if (!otp) {
    throw new BadRequestException(
      'AUTH_OTP_EXPIRED',
      'Verification code has expired. Please request a new one.'
    );
  }

  if (otp.attempts >= otp.max_attempts) {
    throw new BadRequestException(
      'AUTH_OTP_MAX_ATTEMPTS',
      'Too many incorrect attempts. Please request a new code.'
    );
  }

  if (otp.code !== normalizedCode) {
    await prisma.password_reset_otp.update({
      where: { id: otp.id },
      data: { attempts: { increment: 1 } },
    });

    const remainingAttempts = otp.max_attempts - otp.attempts - 1;
    throw new BadRequestException(
      'AUTH_OTP_INVALID',
      remainingAttempts > 0
        ? `Invalid code. ${remainingAttempts} attempt${remainingAttempts === 1 ? '' : 's'} remaining.`
        : 'Invalid code. No attempts remaining.'
    );
  }

  const resetToken = crypto.randomUUID();

  await prisma.password_reset_otp.update({
    where: { id: otp.id },
    data: {
      verified: true,
      reset_token: resetToken,
    },
  });

  logger.info('OTP verified', { email: normalizedEmail });

  return resetToken;
};

export const consumeResetToken = async (
  email: string,
  token: string
): Promise<boolean> => {
  const normalizedEmail = email.toLowerCase();

  const otp = await prisma.password_reset_otp.findUnique({
    where: { reset_token: token },
  });

  if (!otp || otp.email !== normalizedEmail || !otp.verified) {
    throw new BadRequestException(
      'AUTH_RESET_TOKEN_INVALID',
      'Invalid or expired reset token. Please request a new code.'
    );
  }

  await prisma.password_reset_otp.delete({
    where: { id: otp.id },
  });

  logger.info('Reset token consumed', { email: normalizedEmail });

  return true;
};
